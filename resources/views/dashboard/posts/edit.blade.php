...
<form action="/dashboard/posts/{{ $post->title }}" method="post" class="max-w-sm mx-auto" enctype="multipart/form-data">
    ...
    @method('put')
    <div class="mb-5">
        <label for="base-input" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Produk</label>
    ...
    <div class="mb-5">
        <label for="genre" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Agensi</label>
        <select id="genre" name="genre_id"
            ...
            <option value={{ $genre->id }} {{ old('genre_id', $post->genre->id) == $genre->id ? 'selected' : '' }}>
                {{ $genre->name }}</option>
            @endforeach
        </select>
    </div>
